...
module.exports = function myCustomMacro(arc, cloudformation, stage) {
	...
	authEndpoints.forEach((authorizer) => {
		...
		cloudformation.Resources["HTTP"].Properties["Auth"]["Authorizers"][`Authorizer${authorizer.join('')}`] = {
			IdentitySource: "$request.header.Authorization",
			JwtConfiguration: {
				audience: authorizer.map(auth => ({ Ref: `ArcUserPoolClient${auth}` })),
				issuer: {
		...
		let functionName = `${capitalizeFirstLetter(func[0])}${func[1].split('/').map(el => capitalizeFirstLetter(el)).join('')}`

		// Add authorizer to the http event
		cloudformation.Resources[functionName].Properties.Events[`${functionName}Event`].Properties['Auth'] = {
			Authorizer: `Authorizer${authorizer.join("")}`
		}

		console.log(`Added authorizer ${`Authorizer${authorizer.join('')}`} to ${functionName}`)
	})
	return cloudformation
}
